...
function createImageLifecycle({ ImageUpload, Message, Setting, destroy, cloudName, warn = console.warn, isTrustedUrl = isTrustedImageUrl, pathField = 'imagePath', publicIdField = 'imagePublicId', resourceType = 'image' }) {
  async function withImageUse(username, data, work) {
    ...
    try {
      if (!isTrustedUrl(upload.url, cloudName)) throw new Error('Invalid image upload');
      return await work();
    } finally {
      try {
      ...
      } catch { warn('Media use guard could not be released; retained upload needs review.'); }
    }
  }

  async function cleanupImage(imagePath) {
    if (!isTrustedUrl(imagePath, cloudName)) return;
    try {
      ...
      if (!upload) return;
      const references = { $or: [{ [pathField]: upload.url }, { [publicIdField]: upload.publicId }] };
      // Query after claiming: new uses cannot start until this decision is complete.
      if (await Setting.findOne({ [pathField]: upload.url }) || await Message.findOne(references)) {
      ...
      const result = await destroy(upload.publicId, { resource_type: resourceType, invalidate: true });
      ...
      warn('Media cleanup incomplete; retained upload record needs a cleanup retry.');
    }
  }
