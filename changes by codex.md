...
- Record locally in a browser-supported compressed format (Opus/WebM, Opus/Ogg, or MP4). Nothing uploads before Send. Authenticated, size-limited uploads use Cloudinary's video resource type and incoming MP3/64 kbps/two-minute conversion; server-side URL, format, size, measured duration (with a half-second encoder-padding tolerance), and ownership checks precede message acceptance. The installed SDK's transformation serialization was checked against the requested options.
- Store audio references and duration in MongoDB and audio assets in Cloudinary. Upload records are unique per authenticated user/client ID, uploaded outbox retries reuse the existing asset, and uncertain database acknowledgements do not delete an already-registered recording. Recordings not uploaded yet are held only in this tab's memory.
- Added native Play/Pause/progress/duration controls to voice bubbles with no audio preload. Preserve players during history/reaction/receipt updates; stop removed/unsent clips, coordinate voice/YouTube playback, and retain replies, reactions, read receipts, retry deduplication, and server-side ownership rules.
- After a successful unsend, delete the audio asset and upload record only when no message references it. Reused the guarded media lifecycle with voice-specific URL/public-ID fields and Cloudinary resource type to protect concurrent sends. Cleanup failures retain blocked records for review rather than rolling back unsend; no historical sweep or scheduled retry was added.
- Updated the Git-ignored offline adapter for simulated voice sends and unsend cleanup using local object URLs. The actual frontend recorder can be exercised in the offline preview where the browser permits microphone access; this does not verify live uploads or cross-device codec behavior.
- Backend suite passed, including recorder cancellation/permission/limit cases, audio validation, registration failures and lost acknowledgements, upload reuse, message actions/ownership, referenced-asset preservation, and playback preservation/removal. Follow-up targeted recorder/upload tests passed after final adjustments. JavaScript syntax, transformation serialization, and diff whitespace checks passed. Real microphone/browser visuals, Cloudinary conversion, and two-device integration remain unverified; no browser was available in the session.
- Local only; nothing committed, pushed, or deployed. Deploy both backend and frontend, including `frontend/voice-recorder.js`, then refresh open tabs. New optional message fields and a VoiceUpload collection/index need no message rewrite. Restart all backend instances together so every process uses the media guards.
